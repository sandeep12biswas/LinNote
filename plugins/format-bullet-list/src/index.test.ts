...
/** A fluent chain mock standing in for `Editor.chain()` — records each call in order so a test can assert `focus().toggleBulletList().run()` was invoked, without a real TipTap instance. */
...
describe("core.format.bullet-list", () => {
  it("declares a manifest with the expected id", () => {
    expect(plugin.manifest.id).toBe("core.format.bullet-list");
  });

  it("declares a Format-menu entry pointing at its own command id", () => {
    expect(plugin.manifest.contributes.menu).toEqual([
      { menu: "Format", label: "Bullet List", commandId: "core.format.bullet-list.apply", priority: 25 },
    ]);
  });

  it("toggles a bulleted list on the currently active editor when run", () => {
    const { chain, calls } = makeChainMock();
    vi.mocked(getActiveEditor).mockReturnValue({ chain: () => chain } as never);
    const ctx = makeContext();

    plugin.activate(ctx);
    ctx.registered.get("core.format.bullet-list.apply")?.();
  ...
  it("does nothing (doesn't throw) when no editor is currently active", () => {
    ...
    expect(() => ctx.registered.get("core.format.bullet-list.apply")?.()).not.toThrow();
  });
});
