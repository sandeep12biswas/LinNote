...
/** A fluent chain mock standing in for `Editor.chain()` — records each call in order so a test can assert `focus().toggleTaskList().run()` was invoked, without a real TipTap instance. */
...
describe("core.format.checkbox-list", () => {
    ...
    expect(plugin.manifest.contributes.menu).toEqual([
      {
        menu: "Format",
        label: "Checkbox List",
        commandId: "core.format.checkbox-list.apply",
        priority: 45,
      },
    ]);
  });

  it("toggles TaskList on the currently active editor when run", () => {
    const { chain, calls } = makeChainMock();
    vi.mocked(getActiveEditor).mockReturnValue({ chain: () => chain } as never);
    const ctx = makeContext();

    plugin.activate(ctx);
    ctx.registered.get("core.format.checkbox-list.apply")?.();
  ...
  it("does nothing (doesn't throw) when no editor is currently active", () => {
    ...
    expect(() => ctx.registered.get("core.format.checkbox-list.apply")?.()).not.toThrow();
  });
});
