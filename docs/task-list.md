...
- [ ] [NTA-47](https://sandeep12biswas.atlassian.net/browse/NTA-47) — Phase 9: Performance pass — **partial**: NTA-75/76 done on `feature/module-build` (2026-09-05, commit `51de215`); NTA-73/74 done (2026-09-05, see below); NTA-77 intentionally deferred (see below), so the phase itself isn't closed out yet. RAF batching (NTA-75): `apps/desktop/src/canvas-core/coalescer.ts`'s `apply()` now runs on the next animation frame instead of synchronously on every `update()`, collapsing a fast pointermove stream into one apply per frame. Virtualized segments (NTA-76): `CanvasViewport.tsx` derives a canvas-space `visibleRect` from the render surface's measured size (`ResizeObserver`, same technique as NTA-56's `useElementSize`) and `SegmentLayerHost.tsx` filters against it via the new `viewportCulling.ts` (400-unit overscan margin) — a segment far outside the viewport unmounts from the DOM while its data stays untouched in `useNotePageStore`. Diagnosed and fixed a pre-existing test-flakiness trap along the way: jsdom's `pretendToBeVisual` mode runs its own real `requestAnimationFrame` driver that can race vitest's fake-timer-patched one across sequential tests in one file — `SegmentLayerHost.test.tsx` now stubs `requestAnimationFrame`/`cancelAnimationFrame` directly instead of relying on `vi.useFakeTimers()` for frame timing.
  - [x] [NTA-73](https://sandeep12biswas.atlassian.net/browse/NTA-73) Tiled ink canvases: viewport-intersecting tiles only — done on `feature/module-build` (2026-09-05). `plugins/element-ink/src/ink.ts`'s `computeVisibleTiles` divides canvas-space into a fixed 1024-unit grid (`INK_TILE_SIZE`) and returns only the tiles intersecting the host's `visibleRect` expanded by a 512-unit `INK_TILE_OVERSCAN` (same don't-unmount-at-the-edge reasoning as NTA-76's own overscan margin); `InkLayer.tsx`'s `InkTileCanvas` mounts one `<canvas>` per visible tile.
  - [x] [NTA-74](https://sandeep12biswas.atlassian.net/browse/NTA-74) Static vs. active ink layer split — done alongside NTA-73. Each tile paints only the committed strokes `bucketStrokesByTile` says intersect it, `memo`-wrapped with a shallow stroke-array comparison so an eraser drag (which reallocates every tile's bucket array on every sample) doesn't repaint a tile whose actual contents didn't change. The in-progress pen/highlighter stroke paints onto its own small overlay canvas sized to just its own bounds, repainted every `pointermove` — committed tiles never redraw while it's live. 15 tests in `InkLayer.test.tsx` (up from 11), 8 new in `ink.test.ts`; 54 total in `plugins/element-ink`, 298 in `apps/desktop`; `pnpm typecheck`/`pnpm lint:boundaries`/`pnpm --filter desktop build` all green.
